...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/version-1.9.1-informational" alt="v1.9.1">
</p>
...
**さらに: 2モダリティを融合しても"タダ飯"ではない**（GT 不使用の late fusion, R@1 @ 5 m）:

| seq | LiDAR | Camera | naive RRF | conf-gated |
|---|---|---|---|---|
| 07 | 0.596 | 0.681 | **0.713** | 0.660 |
| 08（**逆方向**） | **0.339** | 0.015 | 0.081 | 0.203 |
| **mean** | 0.704 | 0.709 | 0.695 | **0.714** |

<sub>等重み RRF（**mean 0.695 ＝ 両単体を下回る純損**）は seq08 で盲目カメラが LiDAR を 0.339→0.081 に引きずり下ろす。自己信頼度 gate は **mean 最良 0.714**（決して壊滅しないロバスト性）だが、**seq08 を LiDAR 単体 0.339 まで救えない**（0.203, カメラを 49% 採用）— sequence 内の相対 margin では「カメラが全体的に盲目」を検出できないため。**完全な救済には絶対的・モダリティ横断の信頼度較正が必要** ＝ naive な score 融合では不十分。再現: `python scripts/benchmark_kitti_fusion.py`、詳細は [docs/findings.md](docs/findings.md)。</sub>
